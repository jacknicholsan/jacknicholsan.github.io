(function () {
  ...
  function showWelcome() {
    ...
    if (welcomeElement) {
      ...
      document.head.insertAdjacentHTML(
        "beforeend",
        '<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@700&family=Lexend&display=swap">'
      );
    ...
    if (brandsElement) {
      ...
      if (brandsElement) {
        ...
        const welcomeContainer =
          mainContent.querySelector(".welcome-container");
        if (welcomeContainer) {
    ...
    if (featuresElement) {
      ...
      if (featuresElement) {
        ...
        const paymentContainer =
          mainContent.querySelector(".payment-container");
        if (paymentContainer) {
    ...
    if (bannerElement) {
      ...
      if (bannerElement) {
        ...
        if (tickerContainer) {
          ...
          const featuresContainer = mainContent.querySelector(
            ".features-container"
          );
  ...
  function updateSliderTexts() {
    ...
    const componentTexts = {
      en: {
        ...
        sliderDescription3:
          "Your deposits and withdrawals are instantly credited to your account.",
      },
      tr: {
        ...
        sliderDescription1:
          "Sizlere özel haftalık ödüllerle kazancınızı artırın.",
    ...
    if (sliderDescription1)
      sliderDescription1.textContent = texts.sliderDescription1;
    if (sliderDescription2)
      sliderDescription2.textContent = texts.sliderDescription2;
    if (sliderDescription3)
      sliderDescription3.textContent = texts.sliderDescription3;
  }
    ...
    if (sliderElement) {
      ...
      const componentTexts = {
        ...
        tr: {
          ...
          sliderDescription2: "VIP üyelerimize 7/24 özel destek sağlıyoruz.",
          sliderDescription3:
            "Yatırım ve çekimleriniz anında hesabınıza yansır.",
        },
      };
      const texts = isEnglish ? componentTexts.en : componentTexts.tr;
  ...
  function initFakeBets() {
    ...
    function generateRandomBet() {
      ...
      if (rand < 0.05) {
        // %5 ihtimalle 100-500 USDT
        return Math.floor(Math.random() * 400 + 100);
      } else if (rand < 0.1) {
        // %5 ihtimalle 50-100 USDT
        return Math.floor(Math.random() * 50 + 50);
      } else {
        // %90 ihtimalle 0.20, 1 veya 2 USDT
        const smallBets = [0.2, 1, 2];
    ...
    function addNewBet() {
      ...
      newRow.innerHTML = `
                <td>${
                  pragmaticGames[
                    Math.floor(Math.random() * pragmaticGames.length)
                  ]
                }</td>
                <td>${generateUsername()}</td>
                <td>${formatTime()}</td>
                <td class="text-right">${
                  typeof bet === "number" ? bet.toFixed(2) : bet
                } USDT</td>
                <td class="text-right" style="color: ${
                  multiplier === 0 ? "#ff4444" : "#44ff44"
                }">${multiplier}x</td>
                <td style="color: ${profit < 0 ? "#ff4444" : "#44ff44"}">${
  ...
  function showVIPBanner() {
    document
      .querySelectorAll("h2.section__title.section__title--center")
      .forEach((h2) => {
  ...
  function moveTopGamesAfterBrands() {
    ...
    if (brandContainer && topGamesWrapper) {
      brandContainer.parentNode.insertBefore(
        topGamesWrapper,
        brandContainer.nextSibling
      );
    }
  }
  ...
  function showOriginalGames() {
    ...
    const componentText =
      currentLang == "en" ? "Playico Originals" : "Playico Orijinalleri";
    ...
    if (originalsElement) {
      ...
      if (originalsElement) {
        ...
        if (bannerSection && originalsElement) {
          bannerSection.parentNode.insertBefore(
            originalsElement,
            bannerSection
          );
        }
    ...
    elements.forEach((el) => {
      ["alt", "title"].forEach((attr) => {
        if (el.hasAttribute(attr)) {
          el.setAttribute(
            attr,
            el.getAttribute(attr).replace(/ebetlab/gi, "Playico")
          );
        }
      });
    });
  ...
  function updateSidebarNavIcons() {
    ...
    if (sidebarNavItems.length >= 2) {
      ...
      if (seventhLink) {
        ...
        seventhLink.insertAdjacentHTML(
          "afterbegin",
          `
